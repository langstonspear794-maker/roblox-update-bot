...
class RobloxBot(discord.Client):
    ...
    def _load_data(self) -> None:
        try:
            with open(self.DATA_FILE, "r") as f: d = json.load(f)
            ...
            self._prefix          = d.get("prefix", PREFIX)
    ...
    async def _json(self, url: str) -> typing.Any:
        ...
        except Exception as e: log.warning("fetch_json %s: %s", url, e)
        return None
    ...
    async def _text(self, url: str) -> str | None:
        ...
        except Exception as e: log.warning("fetch_text %s: %s", url, e)
        return None
    ...
    async def poll_ugc_prices(self) -> None:
        if not self._watched_items: return
    ...
    # ================== MESSAGE HANDLER (AUTO‑MOD + PREFIX + APPEALS) ==================
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.guild is None:
            ...
            is_banned = uid in self._banned_users

            # Appeal command for banned users
            if content == "!appeal" or content == "/appeal" or content.startswith("!appeal ") or content.startswith("/appeal "):
                if not is_banned: return await message.channel.send("❌ You are not currently banned.")
                reason = "No reason provided"
                if content.startswith("!appeal "): reason = message.content[len("!appeal "):].strip()
                elif content.startswith("/appeal "): reason = message.content[len("/appeal "):].strip()
                ban_data = self._banned_users[uid]
                guild_id = ban_data.get("guild_id")
                if not guild_id: return await message.channel.send("❌ Could not find the server you were banned from.")
                guild = self.get_guild(guild_id)
                if not guild: return await message.channel.send("❌ That server no longer exists or I’ve been removed.")
                self._ticket_counter += 1
                overwrites = {
                    guild.default_role: discord.PermissionOverwrite(read_messages=False),
                    message.author: discord.PermissionOverwrite(read_messages=False),  # user cannot see channel
                ...
                if not category:
                    try: category = await guild.create_category("Tickets")
                        ...
                        await message.channel.send("❌ I don't have permission to create a category.")
                        return
                try:
                    appeal_ch = await guild.create_text_channel(
                        name=f"appeal-{message.author.id}", category=category, overwrites=overwrites
                    )
                except discord.Forbidden:
                    await message.channel.send("❌ I don't have permission to create a channel.")
                    return
                em = discord.Embed(title="📩 Ban Appeal", description=f"Appeal from **{message.author}** (ID: {message.author.id})\nBan reason: {ban_data.get('reason','N/A')}\nAppeal reason: {reason}", colour=0xe67e22)
                em.set_footer(text="Use !reply <message> to respond. !closeappeal to close.")
                await appeal_ch.send(embed=em)
                await message.channel.send(f"✅ Your appeal has been submitted to **{guild.name}**. Staff will review it soon.")
                self._banned_users[uid]["appeal_channel"] = appeal_ch.id
                self._save_data()
                return

            # Forward DM to appeal channel if banned and channel exists
            if is_banned:
                ban_data = self._banned_users[uid]
                appeal_ch_id = ban_data.get("appeal_channel")
                if appeal_ch_id:
                    guild_id = ban_data.get("guild_id")
                    guild = self.get_guild(guild_id) if guild_id else None
                    ch = guild.get_channel(appeal_ch_id) if guild else None
                    if ch and isinstance(ch, discord.TextChannel):
                        await ch.send(f"**{message.author}**: {message.content}")
                        await message.channel.send("✅ Your message has been sent to the appeal channel.")
                        return
                    else:
                        del self._banned_users[uid]["appeal_channel"]
                        self._save_data()
            ...
            if response is None: response = self._get_chat_response(message.content)
        ...
        # Prefix commands
        if not message.content.startswith(self._prefix): return
        ...
        if cmd == "kick":
            ...
            try:
                ...
                else: self._welcome_message = params
                self._save_data()
        ...
        # --- Appeal reply (staff) ---
            ...
            if not is_mod(): return await send_embed("❌ Permission Denied", "You need moderator permissions.", 0xe63946)
            if not isinstance(channel, discord.TextChannel) or not channel.name.startswith("appeal-"):
                return await send_embed("❌ Error", "This is not an appeal channel.")
            if len(args) < 1: return await send_embed("Usage", f"{self._prefix}reply <message>")
            reply_text = " ".join(args)
            try: user_id = int(channel.name.replace("appeal-", ""))
            except ValueError: return await send_embed("❌ Error", "Could not parse user ID from channel name.")
            user = self.get_user(user_id) or await self.fetch_user(user_id)
            if not user: return await send_embed("❌ Error", "User not found.")
            try: await user.send(f"**Staff Reply**: {reply_text}")
            except discord.Forbidden: return await send_embed("❌ Error", "Cannot send DM to that user.")
            await channel.send(f"**Staff Reply** ({author.mention}): {reply_text}")
            await message.delete()

        # --- Close appeal ---
        elif cmd == "closeappeal":
            if not is_mod(): return await send_embed("❌ Permission Denied", "You need moderator permissions.", 0xe63946)
            if not isinstance(channel, discord.TextChannel) or not channel.name.startswith("appeal-"):
                return await send_embed("❌ Error", "This is not an appeal channel.")
            try: user_id = int(channel.name.replace("appeal-", ""))
            ...
            user = self.get_user(user_id) or await self.fetch_user(user_id)
            if user:
                try: await user.send("🔒 Your appeal has been closed by the moderation team.")
                except Exception: pass
            try: await channel.delete(reason="Appeal closed.")
            except discord.Forbidden: return await send_embed("❌ Error", "I don't have permission to delete this channel.")
            for uid, data in self._banned_users.items():
                if data.get("appeal_channel") == channel.id:
                    del data["appeal_channel"]
                    self._save_data()
                    break
    ...
    def _get_chat_response(self, message: str) -> str:
        ...
        for keyword, responses in CHAT_RESPONSES.items():
            if keyword in msg: return random.choice(responses)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        # --- AI Chat ---
            ...
            async with channel.typing():
                response = await self._chat_with_llm(message.content[len(self._prefix):])
                if response is None: response = self._get_chat_response(message.content[len(self._prefix):])
                await channel.send(response)    # ================== DEEPSEEK AI ==================
    async def _chat_with_llm(self, user_message: str) -> str | None:
        if not DEEPSEEK_API_KEY: return None
        url = "https://api.deepseek.com/v1/chat/completions"
        headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
...
async def cmd_help(interaction: discord.Interaction) -> None:
    ...
    em.set_footer(text=f"Polls every 15 min • @everyone pings {'on' if PING_EVERYONE else 'off'}")
    await interaction.response.send_message(embed=em)
...
async def cmd_announce(interaction: discord.Interaction, channel: discord.TextChannel,
    ...
    bot._command_uses += 1
    try: clr = int(color.lstrip("#"), 16)
    except ValueError: clr = 0x4cc9f0
...
async def cmd_dm_blast(interaction: discord.Interaction, message: str) -> None:
    ...
    if not guild:
        ...
        return
    em = discord.Embed(title=f"📣 Message from {guild.name}", description=message, colour=0x4cc9f0, timestamp=datetime.now(timezone.utc))
...
async def cmd_schedule(interaction: discord.Interaction, channel: discord.TextChannel,
    ...
    bot._scheduled.append({
        "channel_id": channel.id, "title": title, "message": message, "send_at": send_at, "author": str(interaction.user),
    })
    bot._save_data()
    ts = int(send_at)
...
async def cmd_audit(interaction: discord.Interaction) -> None:
    ...
    em = discord.Embed(title="📋 Bot Audit Log (Last 15 Actions)", colour=0x4cc9f0, timestamp=datetime.now(timezone.utc))
    if entries:
        for e in reversed(entries):
            detail = f" — {e['detail']}" if e.get("detail") else ""
            em.add_field(name=f"`{e['action']}` • {e['time']}", value=f"By: **{e['by']}**{detail}"[:1024], inline=False)
    else: em.description = "No audit log entries yet."
    await interaction.response.send_message(embed=em)
...
async def cmd_autorole(interaction: discord.Interaction, role: discord.Role | None = None) -> None:
    ...
    bot._save_data()
    if role: await interaction.response.send_message(f"✅ New members will automatically receive {role.mention}.")
    else: await interaction.response.send_message("✅ Auto-role has been disabled.")
...
async def cmd_role_info(interaction: discord.Interaction, role: discord.Role) -> None:
    ...
    em = discord.Embed(title=f"🎭 Role: {role.name}", colour=role.colour, timestamp=datetime.now(timezone.utc))
    ...
    if perms:
        em.add_field(name="Key Permissions", value=", ".join(perms[:15]) + ("…" if len(perms) > 15 else ""), inline=False)
...
async def cmd_set_prefix(interaction: discord.Interaction, prefix: str) -> None:
    bot._command_uses += 1
    if len(prefix) > 5: await interaction.response.send_message("❌ Prefix must be 5 characters or fewer.", ephemeral=True); return
    bot._prefix = prefix
    bot._save_data()
...
async def cmd_bot_info(interaction: discord.Interaction) -> None:
    ...
    em = discord.Embed(title="⚙️ Full Bot Configuration", colour=0x4cc9f0, timestamp=datetime.now(timezone.utc))
    ...
    em.add_field(name="🔕 Muted", value="Yes" if time.time() < bot._muted_until else "No", inline=True)
    em.add_field(name="📊 Poll Interval", value=f"Every 15 min", inline=True)
    em.add_field(name="👁️ Watched Items", value=str(len(bot._watched_items)), inline=True)
...
async def cmd_reset_settings(interaction: discord.Interaction) -> None:
    ...
    bot._save_data()
    em = discord.Embed(title="🔄 Settings Reset", description="All bot settings have been reset to defaults.", colour=0xe63946, timestamp=datetime.now(timezone.utc))
...
async def cmd_backup_settings(interaction: discord.Interaction) -> None:
    ...
    data = {
        ...
        "prefix": bot._prefix, "log_channel_id": bot._log_channel_id,
        "ping_role_id": bot._ping_role_id, "autorole_id": bot._autorole_id,
        ...
        "scheduled_count": len(bot._scheduled), "audit_entries": len(bot._audit_log),
        "automod_enabled": bot._automod_enabled, "automod_words": bot._automod_words,
        "automod_penalty": bot._automod_penalty, "banned_users": bot._banned_users,
    }
    import io
    buf = io.BytesIO(json.dumps(data, indent=2).encode())
    buf.seek(0)
    fname = f"bot_backup_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    await interaction.followup.send(content="✅ Here is your settings backup:", file=discord.File(buf, filename=fname), ephemeral=True)
...
@app_commands.describe(amount="Number of messages to delete (1–100)", channel="Channel to purge (defaults to current)")
@mod_check()
async def cmd_purge(interaction: discord.Interaction, amount: int, channel: discord.TextChannel | None = None) -> None:
    bot._command_uses += 1
...
@admin_check()
async def cmd_antiraid_on(interaction: discord.Interaction, action: str | None = None) -> None:
    bot._command_uses += 1
    if action and action.lower() in ("kick", "ban"): bot._antiraid_action = action.lower()
    bot._antiraid_enabled = True
    bot._save_data()
    em = discord.Embed(title="🚨 Anti-Raid Lockdown ENABLED", colour=0xe63946, timestamp=datetime.now(timezone.utc))
...
async def cmd_antiraid_off(interaction: discord.Interaction) -> None:
    ...
    bot._save_data()
    em = discord.Embed(title="✅ Anti-Raid Lockdown DISABLED", colour=0x06d6a0, timestamp=datetime.now(timezone.utc))
...
@app_commands.describe(threshold="Number of joins to trigger lockdown (default 10)", window="Time window in seconds (default 10)",
                        action="Action on new joiners during lockdown: kick or ban (default kick)", auto="Enable automatic detection (default true)")
@admin_check()
async def cmd_antiraid_config(interaction: discord.Interaction, threshold: int | None = None, window: int | None = None,
                               action: str | None = None, auto: bool | None = None) -> None:
    bot._command_uses += 1
    if threshold is not None and threshold >= 2: bot._antiraid_threshold = threshold
    if window is not None and window >= 3: bot._antiraid_window = window
    if action and action.lower() in ("kick", "ban"): bot._antiraid_action = action.lower()
    if auto is not None: bot._antiraid_auto = auto
    bot._save_data()
    em = discord.Embed(title="⚙️ Anti-Raid Config Updated", colour=0x9b5de5, timestamp=datetime.now(timezone.utc))
    ...
    await interaction.response.send_message(embed=em)
    bot._add_audit("antiraid_config", interaction.user, f"threshold={bot._antiraid_threshold} window={bot._antiraid_window}s action={bot._antiraid_action} auto={bot._antiraid_auto}")
...
async def cmd_antiraid_status(interaction: discord.Interaction) -> None:
    ...
    em = discord.Embed(title="🛡️ Anti-Raid Status", colour=0xe63946 if bot._antiraid_enabled else 0x06d6a0, timestamp=datetime.now(timezone.utc))
...
async def cmd_automod_enable(interaction: discord.Interaction, penalty: str | None = None) -> None:
    bot._command_uses += 1
    if penalty and penalty.lower() in ("kick", "timeout_week"): bot._automod_penalty = penalty.lower()
    bot._automod_enabled = True
    bot._save_data()
    em = discord.Embed(title="✅ Auto-Mod ENABLED", colour=0x06d6a0, timestamp=datetime.now(timezone.utc))
    em.add_field(name="Strike 1–2", value="10-minute timeout + message deleted", inline=False)
    em.add_field(name="Strike 3+", value="Kicked from server" if bot._automod_penalty == "kick" else "7-day timeout", inline=False)
...
async def cmd_automod_removeword(interaction: discord.Interaction, word: str) -> None:
    bot._command_uses += 1
    w = word.lower().strip()
    if w not in bot._automod_words: await interaction.response.send_message(f"⚠️ `{w}` is not in the filter.", ephemeral=True); return
...
async def cmd_automod_status(interaction: discord.Interaction) -> None:
    bot._command_uses += 1
    em = discord.Embed(title="🤬 Auto-Mod Status", colour=0x06d6a0 if bot._automod_enabled else 0xe63946, timestamp=datetime.now(timezone.utc))
    ...
    em.add_field(name="Strike 3+", value="Kicked" if bot._automod_penalty == "kick" else "7-day timeout", inline=False)
    words_display = ", ".join(f"`{w}`" for w in sorted(bot._automod_words))
    if len(words_display) > 900: words_display = words_display[:900] + "…"
...
@mod_check()
async def cmd_automod_logs(interaction: discord.Interaction, member: discord.Member | None = None, page: int = 1) -> None:
    bot._command_uses += 1
    entries = list(reversed(bot._automod_log))
    if member: entries = [e for e in entries if e["uid"] == str(member.id)]
    per_page = 10; total_pages = max(1, (len(entries) + per_page - 1) // per_page)
    page = max(1, min(page, total_pages))
    slice_ = entries[(page-1)*per_page:page*per_page]
    em = discord.Embed(title="🤬 Auto-Mod Violation Log", colour=0xe63946, timestamp=datetime.now(timezone.utc))
    if not slice_: em.description = "No violations recorded yet."
    else:
        lines = []
        for e in slice_:
            try: ts = datetime.fromisoformat(e["ts"]); ts_str = discord.utils.format_dt(ts, style="R")
            except Exception: ts_str = e["ts"][:16]
            lines.append(f"**{e['user']}** — `{e['word']}` | Strike **{e['strikes']}** | {e['action']} | #{e['channel']} | {ts_str}")
        em.description = "\n".join(lines)
    em.set_footer(text=f"Page {page}/{total_pages} • {len(entries)} total violation(s)")
    if member: em.set_author(name=str(member), icon_url=member.display_avatar.url)
    await interaction.response.send_message(embed=em, ephemeral=True)
...
async def cmd_strike_leaderboard(interaction: discord.Interaction) -> None:
    ...
    for entry in bot._automod_log:
        uid = entry["uid"]; totals[uid] = totals.get(uid, 0) + 1; user_names[uid] = entry["user"]
    for uid, count in bot._automod_strikes.items():
        totals[uid] = max(totals.get(uid, 0), count)
    if not totals: await interaction.response.send_message("📊 No strikes recorded yet — the server is clean!", ephemeral=True); return
    ...
    em = discord.Embed(title="📊 Strike Leaderboard — Repeat Offenders", description="\n".join(lines), colour=0xe63946, timestamp=datetime.now(timezone.utc))
...
@app_commands.describe(reported="The member you are reporting", reason="Describe what happened",
                        attachment="Screenshot or evidence (required)", attachment2="Second screenshot (optional)", attachment3="Third screenshot (optional)")
async def cmd_user_report(interaction: discord.Interaction, reported: discord.Member, reason: str,
                          attachment: discord.Attachment, attachment2: discord.Attachment | None = None,
                          attachment3: discord.Attachment | None = None) -> None:
    bot._command_uses += 1
    if reported.id == interaction.user.id: await interaction.response.send_message("❌ You can't report yourself.", ephemeral=True); return
    if reported.bot: await interaction.response.send_message("❌ You can't report a bot.", ephemeral=True); return
    await interaction.response.defer(ephemeral=True)
    ...
    bot._reports.append({
        ...
        "reported": f"{reported} ({reported.id})", "reported_id": str(reported.id),
        "reason": reason, "attachments": [a.url for a in attachments],
    ...
    def make_embed(show_reporter: bool) -> discord.Embed:
        ...
        em.add_field(name="Reason", value=reason, inline=False)
        if show_reporter: em.add_field(name="Reporter", value=f"{interaction.user.mention} (`{interaction.user}`)", inline=True)
        else: em.add_field(name="Reporter", value="*Anonymous*", inline=True)
        ...
        return em
    log_em = make_embed(show_reporter=True); log_posted = False
        ...
        if isinstance(ch, discord.abc.Messageable):
            try:
                ...
                for att in attachments:
                    data = await att.read(); files.append(discord.File(io.BytesIO(data), filename=att.filename))
                await ch.send(content="@here **New member report received**", embed=log_em, files=files)
                log_posted = True
            except Exception as e: log.warning("Could not post report to log channel: %s", e)
    dm_count = 0; guild = interaction.guild
    if guild and (bot._mod_role_id or bot._admin_role_id):
        target_role_ids = [r for r in [bot._mod_role_id, bot._admin_role_id] if r]; notified_ids: set[int] = set()
        for role_id in target_role_ids:
            ...
            for member in role.members:
                ...
                try:
                    dm_em = make_embed(show_reporter=True); dm_files = []
                    for att in attachments:
                        data = await att.read(); dm_files.append(discord.File(io.BytesIO(data), filename=att.filename))
                    await member.send(content=f"🚨 **New report in {guild.name}** — {report_id}", embed=dm_em, files=dm_files)
                    dm_count += 1
                except Exception: pass
    confirm_em = discord.Embed(
        title="✅ Report Submitted",
        description=f"Your report against **{reported}** has been received.\n**Report ID:** `{report_id}`\n\n"
                    f"{'Moderators have been notified via DM.' if dm_count else ''}"
                    f"{'Report also posted in the mod log channel.' if log_posted else ''}",
        colour=0x06d6a0, timestamp=ts)
...
@app_commands.describe(member="Filter to reports about a specific member (optional)",
                        status="Filter by status: all, open, resolved, dismissed (default: open)", page="Page number (default 1)")
@mod_check()
async def cmd_view_reports(interaction: discord.Interaction, member: discord.Member | None = None, status: str = "open", page: int = 1) -> None:
    bot._command_uses += 1; status = status.lower()
    if status not in ("all", "open", "resolved", "dismissed"):
        await interaction.response.send_message(f"❌ Invalid status. Choose from: all, open, resolved, dismissed", ephemeral=True); return
    entries = list(reversed(bot._reports))
    if member: entries = [e for e in entries if e["reported_id"] == str(member.id)]
    if status != "all": entries = [e for e in entries if e.get("status", "open") == status]
    per_page = 5; total_pages = max(1, (len(entries) + per_page - 1) // per_page)
    page = max(1, min(page, total_pages))
    slice_ = entries[(page-1)*per_page:page*per_page]
    em = discord.Embed(title="📋 Member Reports", colour=0xff6b6b, timestamp=datetime.now(timezone.utc))
    if not slice_: em.description = "No reports found matching those filters."
    else:
        for e in slice_:
            s = e.get("status", "open"); icon = {"open": "🔴", "resolved": "✅", "dismissed": "🔕"}.get(s, "🔴")
            try: ts = datetime.fromisoformat(e["ts"]); ts_str = discord.utils.format_dt(ts, style="R")
            except Exception: ts_str = e["ts"][:16]
            n_att = len(e.get("attachments", []))
            em.add_field(
                name=f"{icon} `{e['id']}` — {e['reported']}",
                value=f"**Reason:** {e['reason'][:120]}{'…' if len(e['reason'])>120 else ''}\n"
                      f"**Status:** {s}  •  **Evidence:** {n_att} file(s)  •  {ts_str}\n"
                      f"**Attachments:** " + (" ".join(f"[{i+1}]({u})" for i, u in enumerate(e.get("attachments", []))) or "none"),
                inline=False)
    em.set_footer(text=f"Page {page}/{total_pages} • {len(entries)} report(s) • Use /resolve_report or /dismiss_report to act")
    if member: em.set_author(name=str(member), icon_url=member.display_avatar.url)
    await interaction.response.send_message(embed=em, ephemeral=True)
...
async def cmd_resolve_report(interaction: discord.Interaction, report_id: str, note: str = "") -> None:
    bot._command_uses += 1; rid = report_id.upper().strip()
    for e in bot._reports:
        if e["id"] == rid:
            e["status"] = "resolved"; e["closed_by"] = str(interaction.user); e["close_note"] = note
            bot._save_data()
            await interaction.response.send_message(f"✅ Report `{rid}` marked as **resolved**." + (f"\nNote: {note}" if note else ""), ephemeral=True)
            bot._add_audit("resolve_report", interaction.user, f"{rid}" + (f" — {note}" if note else "")); return
...
async def cmd_dismiss_report(interaction: discord.Interaction, report_id: str, reason: str = "") -> None:
    ...
    for e in bot._reports:
        if e["id"] == rid:
            e["status"] = "dismissed"; e["closed_by"] = str(interaction.user); e["close_note"] = reason
            bot._save_data()
            await interaction.response.send_message(f"🔕 Report `{rid}` **dismissed**." + (f"\nReason: {reason}" if reason else ""), ephemeral=True)
            bot._add_audit("dismiss_report", interaction.user, f"{rid}" + (f" — {reason}" if reason else "")); return
    await interaction.response.send_message(f"❌ Report `{rid}` not found.", ephemeral=True)
...
async def cmd_verify_slash(interaction: discord.Interaction) -> None:
    if not bot._verified_role_id: await interaction.response.send_message("❌ Verified role not set. Admins use `/setverifiedrole`.", ephemeral=True); return
    role = interaction.guild.get_role(bot._verified_role_id)
    if not role: await interaction.response.send_message("❌ Verified role not found.", ephemeral=True); return
    view = discord.ui.View()
    btn = discord.ui.Button(label="Verify", style=discord.ButtonStyle.green)
    async def callback(inter: discord.Interaction):
        if role in inter.user.roles: await inter.response.send_message("You are already verified.", ephemeral=True); return
        try: await inter.user.add_roles(role); await inter.response.send_message("✅ You are now verified!", ephemeral=True)
        except discord.Forbidden: await inter.response.send_message("❌ I can't assign roles.", ephemeral=True)
    btn.callback = callback; view.add_item(btn)
    em = discord.Embed(title="Verification", description="Click the button below to verify yourself.", colour=0x06d6a0)
...
async def cmd_suggest_slash(interaction: discord.Interaction, suggestion: str) -> None:
    if not bot._suggestion_channel_id: await interaction.response.send_message("❌ Suggestion channel not configured.", ephemeral=True); return
    ch = interaction.guild.get_channel(bot._suggestion_channel_id)
    if not isinstance(ch, discord.TextChannel): await interaction.response.send_message("❌ Suggestion channel not found.", ephemeral=True); return
    ...
    msg = await ch.send(embed=em); await msg.add_reaction("👍"); await msg.add_reaction("👎")
...
@admin_check()
async def cmd_reactionrole_slash(interaction: discord.Interaction, action: str, channel: discord.TextChannel,
                                  message_id: str, emoji: str, role: discord.Role | None = None) -> None:
    try: msg_id = int(message_id)
    except ValueError: await interaction.response.send_message("❌ Invalid message ID.", ephemeral=True); return
    if action.lower() == "add":
        if not role: await interaction.response.send_message("❌ Role is required for add.", ephemeral=True); return
        msg = await channel.fetch_message(msg_id)
        try: await msg.add_reaction(emoji)
        except Exception: await interaction.response.send_message("❌ Cannot add that emoji.", ephemeral=True); return
        bot._reaction_roles.setdefault(str(msg_id), {})[emoji] = role.id; bot._save_data()
        ...
        if str(msg_id) in bot._reaction_roles and emoji in bot._reaction_roles[str(msg_id)]:
            ...
            if not bot._reaction_roles[str(msg_id)]: del bot._reaction_roles[str(msg_id)]
            bot._save_data()
    ...
    else: await interaction.response.send_message("❌ Action must be 'add' or 'remove'.", ephemeral=True)
...
async def cmd_ticket_slash(interaction: discord.Interaction, reason: str = "No reason") -> None:
    guild = interaction.guild; bot._ticket_counter += 1
    overwrites = {
        guild.default_role: discord.PermissionOverwrite(read_messages=False),
    ...
    if not category:
        try: category = await guild.create_category("Tickets")
        except discord.Forbidden: await interaction.response.send_message("❌ Cannot create category.", ephemeral=True); return
    try:
        ch = await guild.create_text_channel(name=f"ticket-{bot._ticket_counter}", category=category, overwrites=overwrites)
    except discord.Forbidden: await interaction.response.send_message("❌ Cannot create channel.", ephemeral=True); return
    em = discord.Embed(title="📩 Ticket Created", description=f"Hello {interaction.user.mention}, your ticket is ready.\nReason: {reason}\nUse `/close` to close.", colour=0x4cc9f0)
    await ch.send(embed=em); bot._save_data()
...
async def cmd_giveaway_slash(interaction: discord.Interaction, prize: str, duration_minutes: int, winners: int) -> None:
    if duration_minutes < 1: await interaction.response.send_message("❌ Duration must be at least 1 minute.", ephemeral=True); return
    end_time = time.time() + duration_minutes * 60
    em = discord.Embed(title="🎉 Giveaway!", description=f"**Prize:** {prize}\nReact with 🎉 to enter!\nEnds: <t:{int(end_time)}:R>", colour=0xf72585)
    await interaction.response.send_message(embed=em); msg = await interaction.original_response()
    await msg.add_reaction("🎉")
...
async def cmd_tempvc_slash(interaction: discord.Interaction, name: str, duration_minutes: int) -> None:
    ...
    except discord.Forbidden: await interaction.response.send_message("❌ Cannot create voice channel.", ephemeral=True)
...
async def cmd_ask(interaction: discord.Interaction, question: str) -> None:
    ...
    if answer is None: answer = bot._get_chat_response(question)
