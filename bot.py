...
async def clearwarnings_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ You need Manage Messages permission.", ephemeral=True)
...
@cmd_dm_blast.error
async def dm_blast_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_set_log_channel.error
async def log_channel_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_audit.error
async def audit_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ You need Manage Messages permission.", ephemeral=True)
...
@cmd_set_ping_role.error
async def ping_role_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_autorole.error
async def autorole_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_set_prefix.error
async def prefix_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_antiraid_on.error
async def antiraid_on_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
@cmd_antiraid_config.error
async def antiraid_config_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
...
async def cmd_automod_logs(interaction: discord.Interaction,
                           ...
                           page: int = 1):
    bot._command_uses += 1
    entries = list(reversed(bot._automod_log))   # newest first
    if member:
...
async def cmd_strike_leaderboard(interaction: discord.Interaction):
    bot._command_uses += 1
    # Combine live in-memory strikes with historical totals from the log
    totals: dict[str, int] = {}
    ...
    # Also add any current in-memory strikes not yet in the log
...
async def cmd_user_report(
    ...
    # Block reporting yourself or bots
    ...
    # Post in log channel (shows reporter to mods)
    ...
    # DM every member who has the mod role
    dm_count = 0
    ...
    # Confirm to reporter (anonymous — no mention of who they are)
...
async def cmd_view_reports(
    ...
    entries = list(reversed(bot._reports))   # newest first
    if member:
